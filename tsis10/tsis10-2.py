...
cur = conn.cursor()
cur.execute("SELECT EXISTS(SELECT * FROM information_schema.tables WHERE table_name='tablo')")
table_exists = cur.fetchone()[0]
if not table_exists:
    tablo_creation = '''CREATE TABLE tablo(
        id SERIAL PRIMARY KEY,
        player VARCHAR(255),
        user_score INT
        );'''
    cur.execute(tablo_creation)
    conn.commit()

nickname = input('enter you nickname: ')

cur.execute("SELECT user_score FROM tablo WHERE player = %s", (nickname,))    
result = cur.fetchone()
if result:
    cur.execute("SELECT user_score FROM tablo WHERE player = %s", (nickname,))
    number = cur.fetchone()[0]
    print(f'welcome back, your score is {number}')
else:
    print("Welcome to the game, new user")
    cur.execute('INSERT INTO tablo (player, user_score) VALUES (%s, %s)', (nickname, 0, ))
conn.commit()
...
dy=0
score= int(number) #the main score
score_check = 0 # score that is needed to raise the games speed
count_sp = int(int(number) // 3) #speed counter with it show the level of the game
check = 0
sql_check = 0
                ...
                if event.key == pygame.K_UP and dy != 1: #all  of the second conditions are needed to not allow snake go into itself
                    ...
                    if sql_check == 0:
                        for i in range(0, int(number) - 2):
                            snek.body.append(point(x = (WIDTH // block // 2) - block, y = HEIGHT // block // 2))
                        sql_check = 1
                    ...
                    cur.execute("UPDATE tablo SET user_score = %s WHERE player = %s", (score, nickname))
                    conn.commit()
                    print('Save completed, see you soon!')
                    done = True
    screen.fill(BLACK)
    snek.draw()
    snek.move(dx, dy)
